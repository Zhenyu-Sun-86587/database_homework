...
![alt text](assets/images/b1f42927e0348af8f645a7c071a10eaa.png)
##### 第一层数据流图：

![alt text](assets/images/e00543cf135932b9805d311717c5f1e7.png)
...
系统在数据库层面实现了5个核心触发器，用于自动化处理库存更新、报警生成等业务逻辑，确保数据的一致性和实时性。
...
```sql
DELIMITER $$
CREATE TRIGGER trigger_empty_stock_alert
AFTER UPDATE ON biz_inventory
FOR EACH ROW
BEGIN
    -- 当库存从>0变为0时触发售罄紧急预警
...
DELIMITER ;
```
**说明**：该触发器监控库存表（`biz_inventory`）的更新操作。当某商品的库存从大于0变为0时，自动生成紧急售罄预警，提醒运维人员立即补货。
...
- **增加数据**
  ...
  ![新增商品表单](assets/images/image-10.png)
  ![新增成功展示](assets/images/image-11.png)

- **查询数据**
  在顶部的搜索框中输入关键词（如“可口”），系统会实时过滤并显示匹配的商品记录，支持模糊查询。
  ![查询商品](assets/images/image-15.png)

- **修改数据**
  点击对应商品的“编辑”按钮，修改商品信息（如调整售价），保存后列表数据即时更新。
  ![编辑商品](assets/images/image-13.png)
  ![修改后展示](assets/images/image-14.png)

- **删除数据**
  点击“删除”按钮，确认操作后，该商品记录将从系统中移除。
  ![删除商品](assets/images/image-12.png)
